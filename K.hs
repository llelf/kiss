{-#language FlexibleInstances,NoMonomorphismRestriction,ParallelListComp,PatternSynonyms,PostfixOperators,ScopedTypeVariables,
            StandaloneDeriving,TupleSections,TypeApplications,TypeOperators,ViewPatterns#-}
module K where
...
rev=reverse;fmt=printf;(∅)=mempty;fi=fromIntegral;er=φ∘Er;nyi=er∘("nyi."⊗);wow=φ∘R;such=wow;meh=error"xkcd.com/292"
...
instance Num N where(+)=ltn2(+);(-)=ltn2(-);(*)=ltn2(*);abs=ltn abs;signum=ltn signum;fromInteger=J∘fi
ltn(+)(J a)=J$(+)a;ltn2(+)(J a)(J b)=J$a+b; φn::S->(N->N->N)->E->E->M E;φn _(×)(A(N a))(A(N b))=π∘A∘N$a×b;φn s _ _ _=er(s⊗"₂.typ")
(-^)(A(N a))=π∘A∘N$(-a);(-^)_=er"-₁.typ"

class Ix'd a where{kvs::a->[(E,E)]}; instance Ix'd[E]where kvs=zip$A∘N∘fi<$>[0..]
...
tak(A(N(J i)))|i>=0=wow∘Ls∘take(fi i)∘cycl|T=such∘Ls∘rev∘take(-fi i)∘rev;tak _=π$nyi"#₂case"; cycl[]=[];cycl x=cycle x
jin x y=wow∘Ls$ls x⊗ls y where ls(Ls a)=a;ls a=[a]
siz(Ls a)=R∘A∘N∘fi∘len$a;siz _=Er"rank"
...
instance PP Fun where pp(Op o)=pp o;pp(Lam a b)=fmt"{[%s]%s}"(semi a)(pp b);pp Adv'd{}=error"pp:f:adv"
instance PP L   where pp(N(J x))=sw x⊗"j";pp(C c)=fmt"\"%c\""c;pp(Sy s)='`':s
